...
function Accueil() {
  ...
  const loadSchools = async () => {
    try {
      ...
      const topRated = [...schools].sort((a, b) => b.note - a.note).slice(0, 6);
  ...
  const performLocalSearch = useCallback(
    (searchTerm, currentFilters) => {
      ...
      if (currentFilters.ville) {
        const normalizedVille = removeAccents(
          currentFilters.ville.toLowerCase()
        );
        results = results.filter((school) => {
      ...
      if (currentFilters.specialite) {
        const normalizedSpecialite = removeAccents(
          currentFilters.specialite.toLowerCase()
        );
        results = results.filter((school) => {
          ...
          return school.specialites.some((specialite) => {
            const normalizedSchoolSpecialite = removeAccents(
  ...
  const clearFilter = useCallback(
    (filterType) => {
      ...
      if (
        !searchQuery.trim() &&
        !updatedFilters.ville &&
        !updatedFilters.specialite &&
        !updatedFilters.type
      ) {
        setIsSearching(false);
        setSearchResults([]);
  ...
  const hasActiveSearch =
    isSearching && (searchQuery.trim() || hasActiveFilters);
